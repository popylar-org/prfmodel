...
def predict_contrast_sensitivity(  # noqa: PLR0913 (too many arguments)
    ...
    width_l : float or Tensor
        Curvature of the left branch of the aLP CSF (for spatial frequencies below ``sf_peak``).
        Scalar or tensor broadcastable to ``(num_batches, num_frames)``.
    width_r : Tensor
        Curvature of the right branch of the aLP CSF (for spatial frequencies at or above ``sf_peak``),
    ...
    for _name, _arg in [("cs_peak", cs_peak), ("sf_peak", sf_peak), ("width_r", width_r)]:
        if hasattr(_arg, "ndim") and _arg.ndim < _EXPECTED_NDIM:
            raise ShapeError(_name, tuple(_arg.shape), f"must have at least {_EXPECTED_NDIM} dimensions")

    _batched = [
        (_name, _arg)
        for _name, _arg in [("cs_peak", cs_peak), ("sf_peak", sf_peak), ("width_r", width_r)]
        if hasattr(_arg, "shape") and len(_arg.shape) >= _EXPECTED_NDIM and _arg.shape[0] != 1
    ]
    if _batched:
        _ref_name, _ref_arg = _batched[0]
        for _name, _arg in _batched[1:]:
            if _arg.shape[0] != _ref_arg.shape[0]:
                raise ShapeMismatchError(_ref_name, tuple(_ref_arg.shape), _name, tuple(_arg.shape))
...
class CSFModel(CanonicalCSFModel):
    ...
    ----------
    width_l : float, default=0.68
        Curvature of the left branch of the asymmetric log-parabolic (aLP) CSF. Not fitted.
    %(model_impulse)s
    %(model_scaling)s
    %(model_regressors)s
