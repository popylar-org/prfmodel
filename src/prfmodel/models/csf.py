...
def predict_contrast_sensitivity(  # noqa: PLR0913 (too many arguments)
    ...
    Notes
    -----
    The asymmetric log-parabolic CSF is:
    ...
    for _name, _arg in [("csp", csp), ("sfp", sfp), ("width_r", width_r)]:
        if hasattr(_arg, "ndim") and _arg.ndim < _EXPECTED_NDIM:
            raise ShapeError(_name, tuple(_arg.shape))

    _batched = [
        (n, a)
        for n, a in [("csp", csp), ("sfp", sfp), ("width_r", width_r)]
    ...
    csp = ops.convert_to_tensor(csp, dtype=dtype)  # (num_batches, 1)
    sfp = ops.convert_to_tensor(sfp, dtype=dtype)  # (num_batches, 1)
    ...
    log_sfp = ops.log(sfp) / log10
    log_csp = ops.log(csp) / log10
    log_diff_sq = ops.square(log_sf - log_sfp)

    # Asymmetric branches of the aLP CSF
    sensitivity_l = ops.power(10.0, log_csp - log_diff_sq * ops.square(width_l))
    sensitivity_r = ops.power(10.0, log_csp - log_diff_sq * ops.square(width_r))

    # Select branch: left where sf < sfp, right where sf >= sfp
    return ops.where(sf < sfp, sensitivity_l, sensitivity_r)
...
def predict_contrast_response(
    ...
    # Naka-Rushton: R = c^q / (c^q + Q^q)  where Q = 1 / sensitivity
    c_q = ops.power(ops.maximum(contrast, eps), slope_crf)
    q_q = ops.power(ops.maximum(1.0 / sensitivity, eps), slope_crf)
...
class CSFResponse(BaseResponse[CSFStimulus]):
    ...
    def parameter_names(self) -> list[str]:
        """Names of parameters used by the model: ``csp``, ``sfp``, ``width_r``, ``slope_crf``."""
        return ["csp", "sfp", "width_r", "slope_crf"]
    ...
    def __call__(self, stimulus: CSFStimulus, parameters: pd.DataFrame, dtype: str | None = None) -> Tensor:
        ...
        parameters : pandas.DataFrame
            ...
            for different voxels. Must contain the columns ``csp``, ``sfp``, ``width_r``, and ``slope_crf``.
        dtype : str, optional
            The dtype of the prediction result. If ``None`` (the default), uses the dtype from
            :func:`prfmodel.utils.get_dtype`.
        ...
        """
        dtype = get_dtype(dtype)
        csp = convert_parameters_to_tensor(parameters[["csp"]], dtype=dtype)
        sfp = convert_parameters_to_tensor(parameters[["sfp"]], dtype=dtype)
