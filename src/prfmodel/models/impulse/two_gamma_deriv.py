...
class DerivativeTwoGammaImpulse(BaseImpulse):
    ...
    def parameter_names(self) -> list[str]:
        ...
        Parameter names are: `shape_1`, `rate_1`, `shape_2`, `rate_2`, `weight`, `deriv_weight`.

        """
        return ["shape_1", "rate_1", "shape_2", "rate_2", "weight", "deriv_weight"]
    ...
    def __call__(self, parameters: pd.DataFrame, dtype: str | None = None) -> Tensor:
        ...
        deriv_weight = convert_parameters_to_tensor(parameters[["deriv_weight"]], dtype=dtype)
        # Compute unnormalized density because normalizing constant cancels out when taking difference anyway
        dens_1 = gamma_density(frames, shape_1, rate_1, norm=True)
        # dens_1_norm = dens_1 / ops.max(dens_1, axis=1, keepdims=True)
        dens_2 = gamma_density(frames, shape_2, rate_2, norm=True)
        dens_deriv_1 = derivative_gamma_density(frames, shape_1, rate_1)
        dens_deriv_2 = derivative_gamma_density(frames, shape_2, rate_2)
        # dens_2_norm = dens_2 / ops.max(dens_2, axis=1, keepdims=True)
        dens = dens_1 - weight * dens_2
        dens_deriv = dens_deriv_1 - weight * dens_deriv_2
        return normalize_response(dens + deriv_weight * dens_deriv)
