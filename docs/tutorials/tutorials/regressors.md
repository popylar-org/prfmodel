...
and are expected to be in the space of the neural response (e.g., ). Drift, motion, and physiological signals are
...
canonical model, we wrap our regressor submodels in a {py:class}`prfmodel.regressors.RegressorsList` to chain them
together and supply as the `regressors_model` argument.

```{code-cell} ipython3
from prfmodel.impulse import DerivativeTwoGammaImpulse
from prfmodel.models.prf import Gaussian2DPRFModel
from prfmodel.regressors import AdditiveRegressors, ConvolvedRegressors, RegressorsList
...
prf_model = Gaussian2DPRFModel(
    ...
    regressors_model=RegressorsList([
        AdditiveRegressors(names=["reg_phys"]),  # Regressor names can be arbitrary but must match columns names supplied at prediction
        ConvolvedRegressors(
            names=["reg_event"],
...
Again, we can make a prediciton with the least squares parameters and compare it against the true timecourse.
...
We see that the least-squares-based prediction almost perfectly aligns with the true timecourse.

If we compare the least squares parameters against the true parameters, we see that they also align.
