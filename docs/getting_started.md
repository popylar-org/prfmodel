...
convolved with our pRF response. The two-gamma parameters (`delay`, `dispersion`,
...
so we only set `weight_deriv` here. The parameters `baseline` and `amplitude` shift and scale our convolved
