...
HRF is deliberately excluded from the baseline comparison.  prfpy uses
nilearn's SPM HRF basis set (a weighted combination of the canonical HRF, its
time derivative, and its dispersion derivative), while prfmodel uses a custom
double-gamma with six explicit parameters.  These two formulations cannot be
equated by a simple parameter mapping, so comparing HRF-convolved outputs would
conflate spatial and temporal modelling differences.  The pre-HRF comparison
isolates the spatial encoding step — the meaningful invariant between packages.
