...
class TestPredictContrastSensitivity(TestSetup):
    ...
    def test_shape_error_csp(self, sf: np.ndarray, sfp: np.ndarray, width_r: np.ndarray) -> None:
        """Test that ShapeError is raised when csp has fewer than 2 dimensions."""
        with pytest.raises(ShapeError):
            predict_contrast_sensitivity(sf, np.array([100.0]), sfp, 0.68, width_r)

    def test_shape_error_sfp(self, sf: np.ndarray, csp: np.ndarray, width_r: np.ndarray) -> None:
        """Test that ShapeError is raised when sfp has fewer than 2 dimensions."""
        with pytest.raises(ShapeError):
            predict_contrast_sensitivity(sf, csp, np.array([3.0]), 0.68, width_r)

    def test_shape_error_width_r(self, sf: np.ndarray, csp: np.ndarray, sfp: np.ndarray) -> None:
        """Test that ShapeError is raised when width_r has fewer than 2 dimensions."""
        with pytest.raises(ShapeError):
            predict_contrast_sensitivity(sf, csp, sfp, 0.68, np.array([1.28]))
    ...
    def test_batch_dimension_error(self, sf: np.ndarray) -> None:
        ...
        csp_2 = np.array([[100.0], [150.0]])  # (2, 1)
        sfp_3 = np.array([[2.5], [3.0], [4.0]])  # (3, 1)
        width_r_2 = np.array([[1.28], [1.5]])  # (2, 1)
        with pytest.raises(BatchDimensionError):
            predict_contrast_sensitivity(sf, csp_2, sfp_3, 0.68, width_r_2)
    ...
    def test_output_shape(
        ...
        result = predict_contrast_sensitivity(sf, csp, sfp, 0.68, width_r, dtype=dtype)
        assert result.shape == (self.num_voxels, self.num_frames)

    def test_peak_sf_equals_csp(self) -> None:
        """Test that sensitivity equals csp when sf equals sfp."""
        sfp_val, csp_val = 3.0, 100.0
        sf = np.array([sfp_val])
        csp = np.array([[csp_val]])
        sfp = np.array([[sfp_val]])
        width_r = np.array([[1.28]])

        result = float(np.asarray(predict_contrast_sensitivity(sf, csp, sfp, 0.68, width_r)).flat[0])

        assert np.allclose(result, csp_val)

    def test_left_vs_right_branch(self, sf: np.ndarray) -> None:
        """Test that wider left branch yields lower sensitivity for sf below sfp."""
        sfp_val = 3.0
        csp = np.array([[100.0]])
        sfp = np.array([[sfp_val]])
        width_r = np.array([[1.28]])

        result_wide_l = np.asarray(predict_contrast_sensitivity(sf, csp, sfp, 10.0, width_r))
        result_narrow_l = np.asarray(predict_contrast_sensitivity(sf, csp, sfp, 0.1, width_r))

        # Wider left branch drops sensitivity faster → lower values left of peak
        left_frames = sf < sfp_val
...
class TestCSFResponse(TestSetup):
    ...
    def test_parameter_names(self) -> None:
        """Test that parameter names are ['csp', 'sfp', 'width_r', 'slope_crf']."""
        model = CSFResponse()
        assert model.parameter_names == ["csp", "sfp", "width_r", "slope_crf"]
